...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background pi subagent: a fully autonomous, headless agent with its own context window that inherits this environment's tools and config. Fire-and-forget: this returns immediately with an id. The subagent's final output is queued back to you as a message when it settles, or collect it explicitly with subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Max 4 subagents can be running at once.";
